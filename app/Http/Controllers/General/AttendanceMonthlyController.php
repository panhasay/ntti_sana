...
class AttendanceMonthlyController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->page_id = "10001";
        $this->page = "attendance-monthly";
        $this->prefix = "attendance-monthly";
    ...
    public function index(){
        ...
        return view ('general.attendance_monthly',compact('classes'));
    }

    public function attendanceMonthlyClassList(Request $request)
    {
        ...
        return view('general.attendance_monthly_class_list', compact('records'))->render();
    }

    private function buildAttendanceMonthlyData($class_code, $semester, $years, $id)
    {
    ...
    public function attendanceMonthlyList(Request $request)
    {
        $records = $this->buildAttendanceMonthlyData(
            $request->class_code,
            $request->semester,
            $request->years,
            $request->id
        ...
        return view('general.attendance_monthly_list', $records);
    }

    public function printAttendanceMonthly(Request $request)
    {
        $records = $this->buildAttendanceMonthlyData(
            $request->class_code,
            $request->semester,
        ...
        return view('general.attendance_monthly_sub_lists', $records);
    }

    public function attendanceMonthlySubjectDetail(Request $request)
    {
        $assign_no = $request->assign_no;

        $records = DB::table("student_score as ss")
            ->where("assign_line_no", $assign_no)
            ->join('student as s', 's.code', '=', 'ss.student_code')
            ->selectRaw("
                s.name_2 as student_name,
                s.name as name_en,
                s.gender,
                SUM(CASE WHEN ss.att_score = 0.5 THEN 1 ELSE 0 END) as permission,
                SUM(CASE WHEN ss.att_score = 0 THEN 1 ELSE 0 END) as absent
            ")
            ->groupBy('s.name_2', 's.gender','s.name')   
            ->orderBy('s.name_2', 'asc')                 
            ->orderBy('s.name', 'asc')                  
            ->get();
