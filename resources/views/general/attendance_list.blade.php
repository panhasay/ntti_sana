                    ...
                    <div class="col-md-6 col-sm-6 col-6">
                        <div class="page-title page-title-custom">
                            <div class="title-page">
                                ...
                                បញ្ជីសរុបវត្តមានប្រចាំឆមាសទី {{ \App\Service\Service::convertToKhmerNumber($semester) }}
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-6 col-6">
                ...
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th rowspan="4" width="170" class="text-center fw-bolder">គោត្តនាម-នាម</th>
                            ...
                            @endforeach
                            <th colspan="2" class="text-center fw-bolder">សរុប</th>
                        </tr>
                        <tr>
                    ...
                    <tbody>
                        @foreach ($uniqueStudents as $record)
                            <tr>
                                ...
                                @for ($i = 0; $i < count($attendanceMonths); $i++)
                                    @php
                                        $att_month = $attendanceMonths[$i]->att_month;
                                        $att_year = $attendanceMonths[$i]->att_year;

                                        $matchingRecords = $records->filter(function ($item) use (
                                            $record,
                                            $att_month,
                                            $att_year,
                                        ) {
                                            return $item->student_code === $record->student_code &&
                                                $item->att_month === $att_month &&
                                                $item->att_year === $att_year;
                                        });
...
<script>
    ...
    $(document).on('click', '#YesPrints', function() {
        ...
        let url = '/exam-credit/print-attendance-list?' +
            'class_code=' + encodeURIComponent(code) +
    ...
    $(document).ready(function() {
        const downloadUrl =
            `/exam-credit/attendance-list/download-excel?class_code={{ $classCode }}&semester={{ $semester }}&years={{ $years }}`;
