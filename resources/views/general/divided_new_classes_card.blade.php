...
@section('content')
                                ...
                                <select class="js-example-basic-single FieldRequired" id="class_code" name="class_code"
                                    ...
                                    @foreach ($classs as $record)
                                        <option value="{{ $record->code ?? '' }}"
                                            {{ isset($records->code) && $records->code == $record->code ? 'selected' : '' }}>
                            ...
                            <div class="col-sm-9">
                                <select class="js-example-basic-single FieldRequired" id="school_year_code"
                                    ...
                                    @foreach ($school_years as $record)
                                        <option value="{{ $record->code ?? '' }}"
                                            {{ isset($records->school_year_code) && $records->school_year_code == $record->code ? 'selected' : '' }}>
                                            {{ isset($record->name) ? $record->name : '' }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="js-example-basic-single" id="level" name="level" style="width: 100%;"
                                    ...
                                    @foreach ($qualifications as $value => $label)
                                        <option value="{{ $label->code }}"
                                            {{ isset($records->level) && $records->level == $label->code ? 'selected' : '' }}>
                                            {{ $label->code ?? '' }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <div class="container-fluid p-2">
        <div class="row">
            <div class="col-md-12 col-sm-12 col-6">
                <a class="btn btn-primary btn-icon-text btn-sm mb-2 mb-md-0 me-2" id="GetStudnetRegister"
                    href="javascript:void(0);"><i class="mdi mdi-account-plus"></i> Add New</a>
                ...
                <span class="title-report">
                    ព័ត៍មាននិស្សិតថ្នាក់ {{ $records->code ?? '' }} សរុបចំនួន
                    ​{{ count($record_sub_lines ?? '') }} / 55 នាក់ ស្រី
                    {{ $totalFemale }} នាក់
                </span>
            </div>
        </div>
    ...
    <div class="modal fade" id="ModalStudnetRegister" tabindex="-1" aria-labelledby="ModalStudnetRegister"
        aria-hidden="true">
        <div class="modal-dialog modal-fullscreen" style="max-width: 100%;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-dark" id="ModalStudnetRegister">ឈ្មោះនិស្សិតឆ្នាំទី១ ក្រុម/ថ្នាក់​
                        {{ $records->code ?? '' }}</h5>
                ...
                <div class="modal-footer">
                    <button type="button" id="btnClose" class="btn btn-secondary khmer_os_b"
    ...
    <div class="modal fade" id="divConfirmationline" tabindex="-1" role="dialog"
        aria-labelledby="divConfirmationline" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-m-header">
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#GetStudnetRegister").on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('.contain-getStudentRegister').html(
                            response); // Update the content of the modal body
                    },
                ...
                $.ajax({
                    ...
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr(
                            'content') // Correct header location
                    },
                    success: function(response) {
                        if (response.status == 'success') {
            ...
            $(".formSublista").on('change', function() {
                ...
                var code = "{{ isset($_GET['code']) ? addslashes($_GET['code']) : '' }}";
                url = `/class-schedule/save-schedule?&name=` + name + `&value=` + value + `&date_name=` +
                    date_name + `&date_type=` + date_type + `&date_room=` + date_room + `&code=` + code;
                $.ajax({
                    type: "POST",
                    url: url,
                    headers: {
            ...
            $(document).on('click', '.BtnEditeacher', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        days.forEach(day => {
                            $(`#subjects_code_${day}`).append(new Option("មុខវិជ្ជា",
                                "", true, true));
                        });
                        ...
                        if (response.status === "success") {
                            $("#teachers").empty();
                            $("#teachers").append(new Option(response.records.teacher.name_2,
                                "", true, true));
                            $.each(response.teachers, function(index, teacher) {
                                let option = new Option(teacher.name_2, teacher.code,
                                    false, false);
                            ...
                            if (days.includes(day)) {
                                ...
                                $(subjectCodeSelector).empty().append(new Option(response
                                    .records.subject.name_2, "", true, true));
                                // Parse and format times
                                let [startHours, startMinutes] = response.records.start_time
                                    .split(":");
                                ...
                                response.subjects.forEach(subject => {
                                    $(subjectCodeSelector).append(new Option(subject
                                        .name, subject.code, false, false));
                                });
                        ...
                        $('#teachers, #subjects_code_monday, #subjects_code_tuesday, #subjects_code_wednesday, #subjects_code_thursday, #subjects_code_friday, #subjects_code_saturday')
                            .select2({
