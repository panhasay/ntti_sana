...
<?php
        ...
        $transfer = App\Models\General\TransferLine::select('student_code')->where('student_code', $record->student_code)->first();
    ?>
<tr id="row{{$record->student_code ?? ''}}"
    class="{{ $hang_of_student->student_code ?? '' == $record->student_code ? 'bg-warning text-white' : '' }} {{ $transfer->student_code ?? '' == $record->student_code ? 'bg-info text-white' : '' }}">
    <td class="">
        <div class="btn-group">
            ...
            <div class="dropdown-menu" style="">
                @if(@$hang_of_student->student_code == @$record->student_code)
                    <a class="dropdown-item" id="hang_of_study" data-code="{{ $record->student_code ?? '' }}" data-type="hang_of_study" href="javascript:;">បន្តកាសិក្សា</a>
                @else
                    <a class="dropdown-item" data-type="transfer" data-code="{{ $record->student_code ?? '' }}" id="student_change_class" href="javascript:;">ប្ដូរវេនសិក្សា</a>
